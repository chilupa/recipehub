...
import { heart, heartOutline, share, time, people, ellipsisVertical } from 'ionicons/icons';
...
const RecipeCard: React.FC<RecipeCardProps> = ({ 
  ...
  return (
    <IonCard button routerLink={`/recipe/${recipe.id}`} style={{ margin: '8px 0' }}>
      <div style={{ padding: '12px 16px 8px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '8px', overflow: 'hidden' }}>
          ...
          {recipe.tags.slice(0, 1).map((tag, index) => (
            <IonChip key={index} color="tertiary" style={{ 
              height: '24px', 
              fontSize: '12px', 
              maxWidth: '100px' 
            }}>
